...
### Strength of S4 (compared to S3)

- multiple dispatch

- validity

- virtual/concrete classes

- virtual classes with no slots (equiv to Java interface)

- multiple inheritance: to be used carefully. Typical use case is to
  put more than 1 interface on top of a class by having it inherit
  1 or more virtual classes with no slots

- class unions (just a special kind of virtual class with no slots)

- what about reference classes? they are rarely needed.
  As a general principle, R objects should have a pass-by-copy semantic
  so passing them to a function is guaranteed to leave the original object
  unmodified. Pass-by-reference semantic is almost never needed and should
  be avoided.

- What you see and feel is not what's inside:

- API vs internals: the Bioconductor approach to S4 object implementation
  is to hide what an object really contains from the end user. The slots
  and what they contain are called the _internals_ of the object and should
  not matter to the end user. What matters is the _semantic_ of an object
  i.e. _what_ it represents and how it can be manipulated (API).

- Even developers who implement a new class by extending an existing class
  should try to do it in a way that is as agnostic as possible.
...
# Design and implementation of S4 classes

## Implementing an S4 class

### 2 approaches

- Extend an existing class (less work)

- Sometimes from scratch (a rare situation)
...
- At a minimum some accessors (getters). Try to reuse existing
  generics instead of introducing new ones e.g.:
    - organism, score (defined in BiocGenerics), strand
    - seqinfo (defined in GenomeInfoDb)

- Plus, depending on the "shape" of the objects, methods for some
  of the core generic functions in base R:

    length, names, subsetting ([, [[), dim, dimnames, c(),
    setters, names<-, subassignment ([<-, [[<-), dimnames<-

- Specialized operations: as methods for existing generics
  (e.g. normalize, defined in BiocGenerics) or for new generics.
...
### _Shape_ of an object

- linear: vector-like, list-like

- rectangular: matrix-like, data-frame-like, SummarizedExperiment
...
They are described by the following "backbones":
    ...
    3) Bioconductor introduces a 3rd type of backbone for rectangular
       objects:
...
Note that there are other approaches to implementing constant-width IRanges
objects.

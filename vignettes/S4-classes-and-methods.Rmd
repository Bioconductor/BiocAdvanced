...
is, has the same length as the input and its i-th element corresponds to the
i-th element in the input.
...
- Summarization functions e.g. `min()`, `max()`, `sum()`, `mean()`, `prod`,
  `rowSums()`, ... (but `pmin()` and `pmax()` do)
...
- By extending the Vector class (only if your objects are vector-like objects),
  you get the above (Vector extends Annotated) + `length()` + the `mcols()`
  getter and setter out-of-the box + access to a mechanism that facilitates
  implemention of subsetting (`[`) and validation (via defining a
  `"parallelSlotNames"` method).

- By extending List (only if your objects are list-like objects) you get all
  the above (List extends Vector) + `elementType()` + `as.list()` + access to
  a mechanism to facilitate implementation of `[[`.

Ask on the bioc-devel mailing list if you need help to find a class that
you could extend.
...
[manuals_url]: http://cran.r-project.org/manuals.html
...
- The _Extending RangedSummarizedExperiment_ section of the
  _SummarizedExperiment_ vignette in the `SummarizedExperiment` package

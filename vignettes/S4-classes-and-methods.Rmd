...
really inside is a powerful concept. More on this later.
...
they extend. More on this later.
...
S4 versions of the People/Employees/Company classes:
...
setValidity("People",
    function(object)
    {
        if (!identical(length(object@name), length(object@age)))
            return("'name' and 'age' slots must have the same length")
        TRUE
    }
)

setClass("Employees", contains="People", slots=c(job="character"))

setValidity("Employees",
    function(object)
    {
        if (!identical(length(object@name), length(object@job)))
            return("'name' and 'job' slots must have the same length")
        TRUE
    }
)

setClass("Company", slots=c(company_name="character", employees="Employees"))

setValidity("Company",
    function(object)
    {
        if (length(object@company_name) != 1 || is.na(object@company_name))
            return("'company_name' slot must be a single non-NA string")
        TRUE
    }
)
```

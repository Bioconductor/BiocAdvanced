...
## Memory management: copy-on-change; garbage collection
...
## Correct, robust, simple, fast: practical techniques 
...
1. Create a new package using `Rcpp::Rcpp.package.skeleton()`. Edit
   the DESCRIPTION file to include `BH` in the `LinkingTo:` field.
...
4. Implement the function using Rcpp structures like
   `CharacterVector`; no need to worry about coercing the argument,
   PROTECTion, etc. Decorate the function with the 
...
5. In _R_, make Rcpp process the attributes; check out

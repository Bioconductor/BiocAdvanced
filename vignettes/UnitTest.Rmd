...
Contract: for non-negative integer-valued `n`, produce a vector of length `n` where the `i`th element is `sqrt(i)`.
...
test_that("fun is correct", {
    expect_identical(sqrt(1:5), fun(5))
})

test_that("fun is robust", {
    expect_identical(numeric(0), fun(0))
    expect_error(fun(-1))
})
```
